...
class TimeRegimeDetector:
    ...
    def _find_bottom_hit(
        ...
        - **slope criterion**: log-log tip slope drops below *slope_threshold*
          after the convective onset.
        ...
        "slope", "boundary", "slope+boundary", or (None, None) if not found.
        """
        ...
        # slope criterion: first frame after convective onset where slope < threshold
        t_slope = None
        valid_slopes = ~np.isnan(slopes)
        after_onset = np.zeros(len(self.t), dtype=bool)
        after_onset[oi:] = True
        slope_mask = valid_slopes & after_onset & (slopes < slope_threshold)
        if slope_mask.any():
            t_slope = self.t[np.where(slope_mask)[0][0]]
        ...
        if t_slope is not None and t_boundary is not None:
            if abs(t_slope - t_boundary) < 1.0:
                return min(t_slope, t_boundary), "slope+boundary"
            elif t_slope < t_boundary:
                return t_slope, "slope"
            else:
                return t_boundary, "boundary"
        elif t_slope is not None:
            return t_slope, "slope"
    ...
    def detect(
        ...
        y_top: int | None = None,
        y_bot: int | None = None,
        bottom_slope_threshold: float = 0.01,
    ):
        """
        ...
        bottom_slope_threshold : float
            Log-log slope below which a finger is considered to have hit the
            bottom (default 0.01).
        """
        ...
        self.t_bottom, self.bottom_source = self._find_bottom_hit(
            slopes,
            slope_threshold=bottom_slope_threshold,
            y_top=y_top,
            y_bot=y_bot,
        )
        ...
        if self.t_bottom is not None:
            print(f"  First finger hits bottom : {_fmt(self.t_bottom)}  [{self.bottom_source}]")
        if self.Ra:
